...
/// Extracts the referenced .bicep file from the 'using' statement
/// </summary>
public static class BicepParamHelper {
    ...
    public static async Task<string> GetReferencedBicepFileAsync(string bicepparamFilePath) {
        ArgumentException.ThrowIfNullOrWhiteSpace(bicepparamFilePath, nameof(bicepparamFilePath));

        if (!File.Exists(bicepparamFilePath)) {
            throw new FileNotFoundException($"Bicepparam file not found: '{bicepparamFilePath}'.");
        }
        ...
        foreach (var line in lines) {
            if (line.StartsWith("using")) {
                var match = Regex.Match(line, @"using\s+'([^']+)'");
                if (match.Success)  {
        ...
        throw new InvalidOperationException(
            $"Could not find a valid 'using' statement in '{bicepparamFilePath}'. " +
            $"Expected format: using './path/to/template.bicep'");
    }
}
